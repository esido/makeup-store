...
const Powder = () => {
  ...
  useEffect(() => {
    ...
    fetch(
      "https://makeup-api.herokuapp.com/api/v1/products.json?product_category=powder"
    )
      .then((res) => res.json())
      .then((data) => {
        setProducts(
  ...
  return (
    <div className="powder-products">
      <div className="container">
        <h1 className="powder-products-title">POWDER</h1>
        {isLoading ? (
          <div className="loader"></div>
        ) : (
          <div className="powder-products-content">
            {products.map((product) => (
              <div className="powder-products-card" key={product.id}>
                ...
                <div className="powder-products-card-content">
                  <h3 className="powder-products-card-title">{product.name}</h3>
                  <span className="powder-products-card-price">
                    {product.price_sign}
                    {product.price}
                  </span>
                  <div className="powder-products-card-action-bar">
                    <button
